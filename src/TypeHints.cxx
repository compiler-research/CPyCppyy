...
void handle_templates(Cppyy::TCppScope_t fn, std::ostringstream &file, int indent = 0) { exit(1); }
...
PyObject *generate_typehints(PyObject *, PyObject *args) {
    ...
    if (Cppyy::IsNamespace(typ)) {
        return PyErr_Format(PyExc_NotImplementedError, "Type hint generation for C++ namespace not implemented");
    }
    if (Cppyy::IsClass(typ)) {
        // TODO: handle inheritance
        code << "class "
            << Cppyy::GetMethodName(typ)
            << ":\n";
        
        std::vector<Cppyy::TCppScope_t> members;
        Cppyy::GetDatamembers(typ, members);
        for (Cppyy::TCppScope_t datamember: members)
            handle_variable(datamember, code, 1);
        
        members.clear();
        Cppyy::GetClassMethods(typ, members);
        code << "\n";
        for (Cppyy::TCppScope_t datamember: members) {
            code << "\n";
            handle_function(datamember, code, 1);
        }
        
        members.clear();
        Cppyy::GetTemplatedMethods(typ, members);
        code << "\n";
        for (Cppyy::TCppScope_t datamember: members) {
            code << "\n";
            handle_templates(datamember, code, 1);
        }

        // FIXME: typedefs, enums, & nexted classes missing
    }
    if (Cppyy::IsTemplate(typ)) {
        return PyErr_Format(PyExc_NotImplementedError, "Type hint generation for C++ template not implemented");
    }
    if (Cppyy::IsTypedefed(typ)) {
        handle_typedef(typ, code);
    }
    if (Cppyy::IsFunction(typ) && !Cppyy::IsMethod(typ) && !Cppyy::IsPureNamespace(Cppyy::GetParentScope(typ))) {
        handle_function(typ, code);
    }
    if (Cppyy::IsVariable(typ) && !Cppyy::IsPureNamespace(Cppyy::GetParentScope(typ)) && !Cppyy::IsClass(Cppyy::GetParentScope(typ))) {
        handle_variable(typ, code);
    }
    if (!code)
    ...
    return PyUnicode_FromString(code.str().c_str());
}
